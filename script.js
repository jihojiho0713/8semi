const stages=[
{name:"웨이퍼 제조",tag:"INGOT",type:"slice"},
{name:"산화",tag:"OXIDIZE",type:"oxide"},
{name:"포토",tag:"PHOTO",type:"photo"},
{name:"식각",tag:"ETCH",type:"etch"},
{name:"증착·이온주입",tag:"LAYER",type:"deposit"},
{name:"금속 배선",tag:"CONNECT",type:"connect"},
{name:"EDS",tag:"TEST",type:"eds"},
{name:"패키징",tag:"PACK",type:"pack"}
];
...
function render(){runCleanups();clearInterval(timer);const s=stages[current];updateHud();$('stageEyebrow').textContent=`${s.tag} · NEW MISSION`;$('stageTitle').textContent=s.name;$('stageNo').textContent=String(current+1).padStart(2,'0');$('visualGuide').innerHTML=processVisuals[current].map((x,i)=>`<div class="visual-step ${i===1?'active':''}"><div class="visual-icon">${x[0]}</div><span class="visual-label">${x[1]}</span></div>${i<2?'<span class="visual-arrow">→</span>':''}`).join('');$('feedback').textContent='';$('nextBtn').classList.toggle('hidden',!states[current]);$('nextBtn').innerHTML=current===7?'결과 보기 <span>→</span>':'다음 공정 <span>→</span>';openBrief()}
